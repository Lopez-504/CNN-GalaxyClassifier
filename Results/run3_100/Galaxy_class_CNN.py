...
def get_sampler_for_subset(subset):
    ...
    # Nota: Accedemos al dataset padre usando los índices del subset
    # Asumimos que tu GalaxyH5Dataset tiene un atributo .labels o puedes iterar

    # Si tu dataset es muy grande, esto es lento. Si tienes las etiquetas en memoria, úsalas directo.
    # Aquí un método genérico seguro:
    targets = []
    for index in subset.indices:
        # Accedemos a la etiqueta sin cargar la imagen entera si es posible
        # Ajusta esto según tu clase GalaxyH5Dataset.
        # Si dataset[i] devuelve (img, label), hacemos:
    ...
    # Ejemplo: Clase A (100 imgs) -> peso 1/100. Clase B (10 imgs) -> peso 1/10.
    ...
    # 5. Crear el Sampler
    # replacement=True es OBLIGATORIO para oversampling (repetir las pocas que hay)
...
class InstanceWhitening(object):
    ...
    def __call__(self, img):
        # Asumimos que 'img' ya es un Tensor de PyTorch (C, H, W)
